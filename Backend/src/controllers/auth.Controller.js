...
import { sendVerificationConfirmationEmail, sendVerificationTokenEmail } from "../services/auth.Service.js";
...
const registerUserController = AsyncWrapper(async (req, res) => {
    ...
    return res.status(201).json({ success: true, message: "Verification link has been sent on given email !" });
});

const verifyAccountController = AsyncWrapper(async (req, res) => {
    const { token } = req.params;

    const user = await userModel.findOne({ verifyToken: token }).select("+role +verifyTokenExpires +isVerified");
    ...
    return res.status(200).json({ success: true, message: "Account Verified Successfully ! Now you can login !" });
});
